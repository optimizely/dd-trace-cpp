...
ExtractedData merge(const std::vector<ExtractedData>& contexts) {
  ...
  const auto dd =
      std::find_if(contexts.begin(), contexts.end(), [&](const ExtractedData& data) {
  ...
  if (other != contexts.end()) {
    ...
    if (result.parent_id != other->parent_id) {
      if (other->datadog_w3c_parent_id && other->datadog_w3c_parent_id != "0000000000000000") {
        ...
        // TODO: Do we add the result.headers_examined for all of the dd ones? Or just the x-datadog-parent-id?
      }
